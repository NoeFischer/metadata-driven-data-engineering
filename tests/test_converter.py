"""End-to-end tests: Excel -> JSON -> validate."""
...
def output_dir(tmp_path):
    return tmp_path / "json_output"


@pytest.mark.skipif(not SAMPLE_XLSX.exists(), reason="sample xlsx not generated")
def test_convert_produces_json_files(output_dir):
...
@pytest.mark.skipif(not SAMPLE_XLSX.exists(), reason="sample xlsx not generated")
def test_generated_json_is_valid(output_dir):
    files = convert(SAMPLE_XLSX, output_dir)
    for fp in files:
...
def test_customers_pipeline_content(output_dir):
    ...
    data = json.loads((output_dir / "customers_raw.json").read_text())
...
def test_orders_pipeline_has_transform(output_dir):
    convert(SAMPLE_XLSX, output_dir)
    data = json.loads((output_dir / "orders_raw.json").read_text())
